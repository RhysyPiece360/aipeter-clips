...
// Define a map to store the timestamp of each request made by the client
const requestTimestamps = new Map();

// Middleware function to check rate limits
const rateLimitMiddleware = (req, res, next) => {
  const clientIP = req.ip; // Use req.ip or any other identifier specific to your setup
  const now = Date.now();
  const thirtySecondsAgo = now - 30 * 1000;

  // Remove timestamps older than 30 seconds to keep the map size manageable
  ...
  if (requestTimestamps.get(clientIP)?.count >= 10) {
    return res.status(429).send('Too many requests. Please try again later.');
  }

  // Update the request count for the client
  const currentTimestamp = requestTimestamps.get(clientIP);
  if (currentTimestamp) {
    currentTimestamp.count += 1;
  } else {
    requestTimestamps.set(clientIP, { count: 1 });
  }

  // Proceed with the actual route handling
  next();
};

// Add the rate limit middleware to your route
app.put('/api/video/like/:videoid', rateLimitMiddleware, async (req, res) => {
